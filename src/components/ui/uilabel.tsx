...
export default function UILabelSelect({defaultValue, name, title}: {
    defaultValue: string,
    name: string,
    title: string
}) {
    ...
    return (
        <>
            <div>
                ...
                <div className="relative mt-2 rounded-md shadow-sm">
                    ...
                    <div className="absolute inset-y-0 right-0 flex items-center">
                        <FontAwesomeIcon icon={faEdit} className="" onClick={() => setLabelModal(true)}/>
                    </div>
                </div>
            </div>
            ...
            >
                ...
                <Dialog.Panel
                    className="fixed inset-y-0 right-24 left-24 m-auto z-50 w-full overflow-y-auto bg-white px-6 py-6 dark:bg-slate-900 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
                    <div className="flex flex-col space-y-4 items-center justify-between">
                        ...
                        <form action={submitRawIDs}>
                            <UIInput type={InputType.text} name="rawIDs" title="Raw IDs" defaultValue={value} hint="Separated by comma"/>
                            <p>{value.split(",").map(e=><span className='bg-blue-200 dark:bg-blue-800'>{e}</span>)}</p>
