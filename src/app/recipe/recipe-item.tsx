...
export function RecipeCard({recipe}: Readonly<{ recipe: Recipe }>) {
    ...
    if (recipe.odCover !== null) {
        img =
            <LazyExtendedImage src={recipe.odCover} alt="Cover" className="rounded-t-lg"/>;
    }
    return (<div
        className="w-[300px] shrink-0 mx-2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 inline-block">
        <Link href={`/recipe/${recipe.publicId}`}>
            {img}
        </Link>
        <div className="p-5">
