...
export default async function RecipeDetail({
    ...
    let titleSection = (<>
        {recipe.odCover !== null ?
            <ExtendedImage src={recipe.odCover} alt="Recipe Cover" className="rounded-xl py-4"/> : <></>}
        <div className="pb-8">
            ...
            <p className="text-gray-600 dark:text-gray-400">
                First published <DateTime date={recipe.createdAt}/>, Last modified <DateTime date={recipe.updatedAt}/>
            </p>
        </div>
    </>), preparationSection = (<>
        ...
        editorialButtons = (
            <div className="pt-16">
                <Link href={"/recipe/edit/" + recipe.publicId}>
                    <button
                        className="px-4 py-2 text-slate-900 rounded hover:text-slate-700 dark:text-slate-100 dark:hover:text-slate-300 border-2 border-slate-900 dark:border-slate-100">
                        Edit
                    </button>
                </Link>
            </div>
